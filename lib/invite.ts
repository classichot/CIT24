...
/** Time-limited demo invites. Default window is 3 days; expired tokens cannot open CIT24.
...
const SECRET = "cit24-invite-v1-7L-advisory";
const INVITE_SESSION_KEY = "cit24-invite-session";
const INVITE_AUTH_KEY = "cit24_invite_auth";
const HOST_SESSION_KEY = "cit24-host-session";
const ISSUED_KEY = "cit24-invites-issued";
const LAST_URL_KEY = "cit24-invite-last-url";
...
export function reviewUrl(token: string) {
  const origin = typeof window !== "undefined" && window.location.origin ? window.location.origin : "https://cit24.vercel.app";
...
export function mintInvite({
  ...
  const payload: InvitePayload = {
    ...
    epoch: INVITE_EPOCH,
    id: uid(),
    mode: mode === "corporate" || mode === "defence" ? mode : "advisor",
