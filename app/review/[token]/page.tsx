...
export default function ReviewInvitePage() {
  ...
  function enter() {
    ...
    router.push(payload.mode === "defence" ? "/evidence" : payload.mode === "corporate" ? "/overview" : "/clients");
  }
  ...
  if (state === "expired") {
    ...
    return (
      <Shell
        ...
        lede="The review window has ended. CIT24 on this URL can no longer be opened. Ask 7L Advisory if you need a new link."
      >
        <p className="eyebrow" style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase" }}>Expired</p>
        <h2>Access ended</h2>
        <p className="text-muted login-card-note">This review link stopped working at {when}. Nothing was filed with the Revenue Department. Demo data stays only in the browser that used the link.</p>
      </Shell>
    );
  }
  ...
  if (state === "revoked") {
    return (
      ...
      >
        ...
        <h2>Link disabled</h2>
        <p className="text-muted login-card-note">Ask 7L for a new invite if you still need to look at CIT24.</p>
      </Shell>
    );
  }
  ...
  return (
    <Shell
      ...
      title="Continue your CIT24 review"
    ...
    >
      <p className="eyebrow" style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase" }}>{payload.label || "Siam Precision demo"}</p>
      <h2>Welcome{existing ? " back" : ""}</h2>
      <p className="text-muted login-card-note">
        You are in a time-limited {payload.mode} review. Open Siam Precision Parts. The engine calculated the figures; this is a demo, not a filing.
      </p>
      <button className="btn btn-primary btn-block" type="button" onClick={enter}>
        Enter {payload.mode === "defence" ? "evidence room" : payload.mode === "corporate" ? "group workspace" : "advisory workspace"}
      </button>
    </Shell>
  );
}
